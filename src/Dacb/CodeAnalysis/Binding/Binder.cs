    ...
    internal sealed class Binder
    {
        ...
        private void BindFunctionDeclaration(FunctionDeclarationSyntax syntax)
        {
            ...
            if (type != TypeSymbol.Void)
                _diagnostics.XXX_ReportFunctionsAreUnsuported(syntax.Type.Span);
        ...
        private BoundStatement BindStatement(StatementSyntax syntax)
        {
            switch (syntax.Kind)
            {
                case SyntaxKind.BlockStatement:
                    return BindBlockStament((BlockStatementSyntax)syntax);
        ...
        private BoundStatement BindBlockStament(BlockStatementSyntax syntax)
        {
            var statements = ImmutableArray.CreateBuilder<BoundStatement>();  
            _scope = new BoundScope(_scope);
            foreach(var statmentSyntax in syntax.Statements)
            {
                var statement = BindStatement(statmentSyntax);
        ...
        {
            var body = BindStatement(syntax.BodyStatement);
            var condition = BindExpression(syntax.Condition, TypeSymbol.Bool);
        ...
        private BoundExpression BindCallExpression(CallExpressionSyntax syntax)
        {
            ...
            for(var i = 0; i < syntax.Arguments.Count; i++)
            {
                ...
                {
                    _diagnostics.ReportWrongArgumentType(syntax.Arguments[i].Span, parameter.Name, argument.Type, parameter.Type);
                    return new BoundErrorExpression();
                }
            }
